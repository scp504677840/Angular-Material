 ...
 *
 * 响应在sidenav容器内滚动事件
 * 要对<mat-sidenav-container>内的滚动做出反应，
 * 您可以通过MatSidenavContainer获取底层的CdkScrollable实例。
 *
 * 语法高亮样式
 * https://prismjs.com/download.html#themes=prism-okaidia&languages=markup+css+clike+javascript+apacheconf+c+cpp+docker+git+go+groovy+http+java+json+kotlin+markdown+lua+nginx+objectivec+sql+properties+python+typescript+swift+yaml&plugins=line-numbers+highlight-keywords
 *
 * 显示行号
...
export class AppComponent implements OnDestroy {
  ...
  get content(): string {
    // return marked(this.contentText);
    return marked('Inline `code` has `back-ticks around` it.\n' +
      '\n' +
      '```javascript\n' +
  ...
  get codeData(): string {
    return '@RestController\n' +
      ...
      '    public Result<List<PermissionInfo>> listPermissions(@PathParam("page") Integer page,\n' +
      '                                                        @PathParam("pageSize") Integer pageSize,\n' +
      '                                                        @PathParam("sort") String sort) {\n' +
      '        return permissionInfoService.listPermissions(new PermissionCondition(sort, page, pageSize));\n' +
      '    }\n' +
      '\n' +
      '    @PostMapping("/permissions")\n' +
      '    public PermissionInfo savePermission(@RequestBody PermissionInfo permissionInfo) {\n' +
      '        PermissionInfo result = permissionInfoService.savePermission(permissionInfo);\n' +
      '        for (int i = 0; i < 20; i++) {\n' +
      '            permissionInfo.setName("0" + i);\n' +
      '            try {\n' +
      '                Thread.sleep(300);\n' +
      '            } catch (InterruptedException e) {\n' +
      '                e.printStackTrace();\n' +
      '            }\n' +
      '            permissionInfoService.savePermission(permissionInfo);\n' +
      '        }\n' +
      '        return result;\n' +
      '    }\n' +
      '\n' +
      '    @DeleteMapping("/permissions/{id}")\n' +
      '    public Result<Boolean> delete(@PathVariable("id") BigInteger id) {\n' +
      '        Result<Boolean> result = new Result<>();\n' +
      '        result.setStatus(HttpStatus.OK.value());\n' +
      '        result.setMessage("删除权限");\n' +
      '        result.setData(permissionInfoService.deletePermissionInfoByPrimaryKey(id));\n' +
      '        return result;\n' +
      '    }\n' +
      '\n' +
      '    @PutMapping("/userInfo")\n' +
