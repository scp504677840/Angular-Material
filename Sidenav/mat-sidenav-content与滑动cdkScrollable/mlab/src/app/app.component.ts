...
export class AppComponent implements AfterViewInit, OnDestroy {
  ...
  /**
   * 重要：需要在AppModule中imports: [ScrollDispatchModule]
   *
   * 可滑动的，使用步骤
   * 1.在可滑动容器上添加CdkScrollable指令；如：<mat-sidenav-content cdkScrollable>
   * 2.在Component中声明@ViewChild(CdkScrollable) scrollable: CdkScrollable;
   * 3.（可选）this.nativeEl = this.scrollable.getElementRef().nativeElement;// 获取DOM元素。
   * 4.（可选）返回在host元素上触发scroll事件时发出的observable。
   * this.scrollable.elementScrolled().subscribe();
   *
   * ScrollDispatchModule是一个服务，在构造函数中导入它，可以调用它的方法获取所有可滑动的DOM。
   */
  ...
  /**
   * 手机、平板和电脑以及其他终端通过MediaMatcher匹配结果。
   */
  ...
  ngAfterViewInit(): void {
    ...
    // 返回在host元素上触发scroll事件时发出的observable。
    this.scrollable.elementScrolled().subscribe(() => {
      console.log(`滚动的纵坐标: ${this.nativeEl.scrollTop}`);
      console.log(`滚动的横坐标: ${this.nativeEl.scrollLeft}`);
      console.log(`可见内容的高度: ${this.nativeEl.clientHeight}`);
      console.log(`滚动容器的总高度: ${this.nativeEl.scrollHeight}`);
      console.log(`滚动容器的总宽度: ${this.nativeEl.scrollWidth}`);

      // 滑动已至底部
      if (this.nativeEl.scrollTop + this.nativeEl.clientHeight === this.nativeEl.scrollHeight) {
        console.log('底部');
      }

      // 滑动已至顶部
      if (this.nativeEl.scrollTop === 0) {
        console.log('顶部');
      }
    });
  }
